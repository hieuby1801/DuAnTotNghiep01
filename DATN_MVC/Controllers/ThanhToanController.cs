        ...
        public async Task<IActionResult> ThanhToan(List<int> MaGioHang)
        {
            ...
            else
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        // Lọc ra các sản phẩm trong giỏ hàng có mã sách trong danh sách masach
                        var giohangchon = await _httpClient.GetAsync($"ThanhToans/LayTTGioHang?maGioHang={string.Join(",", MaGioHang)}&maNguoiDung={idnd}");
                        if (giohangchon.IsSuccessStatusCode)
                        {
                            var jsonResponse = await giohangchon.Content.ReadAsStringAsync();

                            TempData["n"] = MaGioHang;
                            modeltong.GioHangs = JsonConvert.DeserializeObject<List<GioHang>>(jsonResponse);
                        }
